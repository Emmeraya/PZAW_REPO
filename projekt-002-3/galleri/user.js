...
const db_ops = {
  create_user: db.prepare(
    "INSERT INTO pc_users (username, passhash, created_at) VALUES (?, ?, ?) RETURNING id;",
  ),
  get_user: db.prepare(
    "SELECT id, username, created_at FROM pc_users WHERE id = ?;",
  ),
  find_by_username: db.prepare(
    "SELECT id, username, created_at FROM pc_users WHERE username = ?;",
  ),
  get_auth_data: db.prepare(
    "SELECT id, passhash FROM pc_users WHERE username = ?;",
  ),
};
